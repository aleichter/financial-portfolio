...
const Portfolio = require('../src/model/portfolio.js');
let ESDB = require('../src/db/esdb.js');
const {
  InvalidSettlementDate, SecurityDoesNotExistException,
  NegativeSecurityQuantityException, WrongExpectedVersion, AccountDoesNotExist,
} = require('../src/model/exception/domain-exceptions.js');
...
describe('Test suite for Portfolio', () => {
  test('Create and manage a portfolio', async () => {
    ...
    const { portfolioId } = await portfolio.createPortfolio();
  ...
  test('Test portfolio updates with expectedRevision values', async () => {
    ...
    const { portfolioId } = createResponse;
