...
const { AccountDoesNotExist, InvalidSettlementDate, SecurityDoesNotExistException, NegativeSecurityQuantityException } = require("../model/exception/domain-exceptions");
...
const validateSecurity = (portfolioState, accountId, securityId, quantityAdjustment) => {
...
class Portfolio {
    ...
    async createPortfolio() {
        ...
        return { 
            portfolioId: portfolioCreated.get("portfolioId"),
    ...
    async buySecurity(portfolioId, accountId, securityId, quantityAdjustment, cashAdjustment, settlementDate, expectedRevision = null) {
        validateAccountExists(await this.#eventService.load(portfolioId), accountId);
        validateSettlementDate(settlementDate);        
        const securityBought = PortfolioEventService.securityBought(portfolioId, accountId, securityId, quantityAdjustment, cashAdjustment, settlementDate);
        const response = await this.#eventService.save(securityBought, expectedRevision);
        return response.nextExpectedRevision;
    }
    async sellSecurity(portfolioId, accountId, securityId, quantityAdjustment, cashAdjustment, settlementDate, expectedRevision = null) {
        const portfolioState = await this.#eventService.load(portfolioId);
        validateAccountExists(portfolioState, accountId);
        validateSettlementDate(settlementDate);
        validateSecurity(portfolioState, accountId, securityId, quantityAdjustment);
        const securitySold = PortfolioEventService.securitySold(portfolioId, accountId, securityId, quantityAdjustment, cashAdjustment, settlementDate);
        const response = await this.#eventService.save(securitySold, expectedRevision);
        return response.nextExpectedRevision;
    }
    async transferSecurityIn(portfolioId, accountId, securityId, quantityAdjustment, settlementDate, expectedRevision = null) {
        validateAccountExists(await this.#eventService.load(portfolioId), accountId);
        validateSettlementDate(settlementDate);
        const transferredIn = PortfolioEventService.securityTransferredIn(portfolioId, accountId, securityId, quantityAdjustment, settlementDate);
        const response = await this.#eventService.save(transferredIn, expectedRevision);
        return response.nextExpectedRevision;
    }
    async transferSecurityOut(portfolioId, accountId, securityId, quantityAdjustment, settlementDate, expectedRevision = null) {
        const portfolioState = await this.#eventService.load(portfolioId);
        validateAccountExists(portfolioState, accountId);
        validateSettlementDate(settlementDate);
        validateSecurity(portfolioState, accountId, securityId, quantityAdjustment);
        const transferredOut = PortfolioEventService.securityTransferredOut(portfolioId, accountId, securityId, quantityAdjustment, settlementDate);
    ...
    async depositCash(portfolioId, accountId, cashAdjustment, settlementDate, expectedRevision = null) {
        validateAccountExists(await this.#eventService.load(portfolioId), accountId);
        validateSettlementDate(settlementDate);
        const cashDeposited = PortfolioEventService.cashDeposited(portfolioId, accountId, cashAdjustment, settlementDate);
    ...
    async withdrawCash(portfolioId, accountId, cashAdjustment, settlementDate, expectedRevision = null) {
        validateAccountExists(await this.#eventService.load(portfolioId), accountId);
        validateSettlementDate(settlementDate);
        const cashWithdrawn = PortfolioEventService.cashWithdrawn(portfolioId, accountId, cashAdjustment, settlementDate);
    ...
    async receiveDividend(portfolioId, accountId, dividendAmount, settlementDate, expectedRevision = null) {
        validateAccountExists(await this.#eventService.load(portfolioId), accountId);
        validateSettlementDate(settlementDate);
        const dividendReceived = PortfolioEventService.dividendReceived(portfolioId, accountId, dividendAmount, settlementDate);
