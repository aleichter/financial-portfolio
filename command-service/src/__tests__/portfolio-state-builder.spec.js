...
describe("Portfolio test", () => {
    test("PortolioState Test", () => {
        let portfolio = PortfolioBuilder.build("123");
        ...
        portfolio = PortfolioBuilder.updateOrAddAccount(portfolio, account);
        expect(portfolio.get("accounts").get(0).get("accountSecurities").get(0).get("securityId")).toEqual("APPL");
        expect(portfolio.get("accounts").get(0).get("accountSecurities").get(0).get("quantity")).toEqual("10000");

        accountSecurity = PortfolioBuilder.AccountSecurityBuilder.build("GPN", "20000");
        account = PortfolioBuilder.getAccount(portfolio, "1001");
        account = PortfolioBuilder.AccountBuilder.updateOrAddAccountSecurity(account, accountSecurity);
        portfolio = PortfolioBuilder.updateOrAddAccount(portfolio, account);
        expect(portfolio.get("accounts").get(0).get("accountSecurities").get(1).get("securityId")).toEqual("GPN");
        expect(portfolio.get("accounts").get(0).get("accountSecurities").get(1).get("quantity")).toEqual("20000");
        ...
        portfolio = PortfolioBuilder.updateOrAddAccount(portfolio, account);
        expect(portfolio.get("accounts").get(0).get("accountSecurities").get(1).get("securityId")).toEqual("GPN");
        expect(portfolio.get("accounts").get(0).get("accountSecurities").get(1).get("quantity")).toEqual("30000");
        ...
        account = PortfolioBuilder.getAccount(portfolio, "1001");
        let cashAmount = BigInt(account.get("cashAmount")) + 1000n;
